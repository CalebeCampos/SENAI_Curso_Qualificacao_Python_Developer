...
if __name__ == "__main__":
    while True:
        try:
            ...
            match opcao:
                case "1":
                    quantidade_termos = int(input("Informe a quantidade de termos que deseja obter: ").strip())
                    m.limpar_terminal()
                    sequencia_fibonacci = []
                    for i in range(quantidade_termos):
                        termo_fibonacci = m.calcular_termo_fibonacci(i+1)
                        sequencia_fibonacci.append(termo_fibonacci)
